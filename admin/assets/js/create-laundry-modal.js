document.addEventListener("DOMContentLoaded", function () {
  ...
  function addNewOrder() {
    ...
    if (orders.length > 0) {
      newOrderData.data = JSON.parse(
        JSON.stringify(orders[orders.length - 1].data),
      );
    }
  ...
  function duplicateOrderData(sourceOrder, targetOrder) {
    ...
    const targetDryerPreference =
      targetOrder.querySelector(".dryer-preference");
    ...
    const targetDetergentProduct =
      targetOrder.querySelector(".detergent-product");
    ...
    if (detergentProduct && targetDetergentProduct) {
      ...
      setTimeout(() => {
        ...
        const targetScoopsInput = targetOrder.querySelector(
          ".scoops-of-detergent",
        );
    ...
    const sourceClothingSection = sourceOrder.querySelector(
      ".clothing-items-section",
    );
    const targetClothingSection = targetOrder.querySelector(
      ".clothing-items-section",
    );
    const targetToggleButton = targetOrder.querySelector(
      ".toggle-clothing-items",
    );

    if (
      sourceClothingSection &&
      targetClothingSection &&
      !sourceClothingSection.classList.contains("d-none")
    ) {
  ...
  function saveCurrentOrderData(orderIndex) {
    ...
    // Save basic fields
    orderData.roundsOfWash =
      orderElement.querySelector(".rounds-of-wash")?.value || "1";
    orderData.dryerPreference =
      orderElement.querySelector(".dryer-preference")?.value || "0";
    orderData.foldingService =
      orderElement.querySelector(".folding-service")?.checked || false;
    orderData.separateWhites =
      orderElement.querySelector(".separate-whites")?.checked || false;
    orderData.orderRemarks =
      orderElement.querySelector(".order-remarks")?.value || "";
    ...
    orderData.detergentQuantity =
      orderElement.querySelector(".scoops-of-detergent")?.value || "0";
    ...
    orderData.fabconQuantity =
      orderElement.querySelector(".fabcon-cups")?.value || "0";
    ...
    orderData.bleachQuantity =
      orderElement.querySelector(".bleach-cups")?.value || "0";
    ...
    // Save clothing items
    orderData.clothingTops =
      orderElement.querySelector(".clothing-tops")?.value || "0";
    orderData.clothingBottoms =
      orderElement.querySelector(".clothing-bottoms")?.value || "0";
    orderData.clothingUndergarments =
      orderElement.querySelector(".clothing-undergarments")?.value || "0";
    orderData.clothingDelicates =
      orderElement.querySelector(".clothing-delicates")?.value || "0";
    orderData.clothingLinens =
      orderElement.querySelector(".clothing-linens")?.value || "0";
    orderData.clothingCurtains =
      orderElement.querySelector(".clothing-curtains-drapes")?.value || "0";
    orderData.clothingBlankets =
      orderElement.querySelector(".clothing-blankets-comforters")?.value || "0";
    orderData.clothingOthers =
      orderElement.querySelector(".clothing-others")?.value || "0";

    // Save clothing section visibility
    const clothingSection = orderElement.querySelector(
      ".clothing-items-section",
    );
    orderData.clothingSectionVisible =
      clothingSection && !clothingSection.classList.contains("d-none");
  ...
  function displayOrder(orderIndex) {
    ...
    if (
      ...
      Object.keys(orders[orderIndex].data).length > 0
    ) {
    ...
    const separateWhitesCheckbox =
      orderElement.querySelector(".separate-whites");
    if (separateWhitesCheckbox) {
      separateWhitesCheckbox.checked =
        orders[orderIndex].hasLinkedWhitesOrder || false;
    }
  ...
  function loadOrderData(orderElement, data) {
    ...
    const clothingUndergarments = orderElement.querySelector(
      ".clothing-undergarments",
    );
    ...
    const clothingCurtains = orderElement.querySelector(
      ".clothing-curtains-drapes",
    );
    const clothingBlankets = orderElement.querySelector(
      ".clothing-blankets-comforters",
    );
    ...
    if (clothingUndergarments)
      clothingUndergarments.value = data.clothingUndergarments || "0";
    if (clothingDelicates)
      clothingDelicates.value = data.clothingDelicates || "0";
    ...
    if (data.clothingSectionVisible) {
      ...
      if (clothingSection) {
        ...
        if (toggleButton)
          toggleButton.textContent = "Hide Clothing & Household Items";
      }
    }
  }
  ...
  function deleteOrder(orderIndex) {
    ...
    if (orderToDelete.hasLinkedWhitesOrder) {
      const whitesOrderIndex = orders.findIndex(
        (order) =>
          order.linkedToOrder === orderToDelete.orderNumber &&
          order.isWhitesOrder,
      );
  ...
  function handleSeparateWhitesChange(isChecked) {
    ...
    if (isChecked) {
      ...
      const hasLinkedWhitesOrder = orders.some(
        (order, index) =>
          order.linkedToOrder === currentOrder.orderNumber &&
          order.isWhitesOrder,
      );
      ...
      if (!hasLinkedWhitesOrder) {
        ...
        // Show notification
        showNotification(
          `Whites order created! Navigate to Order #${whitesOrderData.orderNumber} to customize it.`,
        );
      }
    } else {
      ...
      if (whitesOrderIndex !== -1) {
        if (confirm("This will delete the linked whites order. Continue?")) {
          ...
          const separateWhitesCheckbox =
            document.querySelector(".separate-whites");
          if (separateWhitesCheckbox) {
  ...
  function getOrderTotal(orderIndex) {
    ...
    if (orderElement) {
      ...
      if (data.detergentProduct) {
        const detergentSelect =
          orderElement.querySelector(".detergent-product");
        const detergentOption = detergentSelect?.querySelector(
          `option[value="${data.detergentProduct}"]`,
        );
      ...
      if (data.fabconProduct) {
        ...
        const fabconOption = fabconSelect?.querySelector(
          `option[value="${data.fabconProduct}"]`,
        );
      ...
      if (data.bleachProduct) {
        ...
        const bleachOption = bleachSelect?.querySelector(
          `option[value="${data.bleachProduct}"]`,
        );
  ...
  function calculateGrandTotal() {
    ...
    if (currentOrderElement) {
      const orderTotal =
        parseFloat(
          currentOrderElement.querySelector(".order-total")?.textContent,
        ) || 0;
  ...
  function convertSavedDataToOrderData(savedData, orderObject) {
    ...
    if (
      !savedData.detergentProduct ||
      !savedData.detergentQuantity ||
      parseInt(savedData.detergentQuantity) < 1
    ) {
    ...
    if (orderElement) {
      ...
      const detergentOption = detergentSelect?.querySelector(
        `option[value="${savedData.detergentProduct}"]`,
      );
      ...
      if (savedData.fabconProduct && parseInt(savedData.fabconQuantity) > 0) {
        const fabconSelect = orderElement.querySelector(".fabcon-product");
        const fabconOption = fabconSelect?.querySelector(
          `option[value="${savedData.fabconProduct}"]`,
        );
        orderData.products.push({
      ...
      if (savedData.bleachProduct && parseInt(savedData.bleachQuantity) > 0) {
        const bleachSelect = orderElement.querySelector(".bleach-product");
        const bleachOption = bleachSelect?.querySelector(
          `option[value="${savedData.bleachProduct}"]`,
        );
        orderData.products.push({
          type: "bleach",
