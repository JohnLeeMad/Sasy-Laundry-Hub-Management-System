...
try {
    ...
    if ($stmt->execute()) {
        ...
        if (file_exists(__DIR__ . '/audit-logger.php')) {
            $auditDescription = 'Added new user: ' . $name . ' (' . $email . ') - Role: ' . $role;
            logActivity($_SESSION['user_id'], $_SESSION['user_role'], $_SESSION['user_name'], 'user_created', $auditDescription);
        }
        ...
        if (file_exists(__DIR__ . '/audit-logger.php')) {
            $errorDescription = 'Failed to add user: ' . $conn->error;
            logActivity($_SESSION['user_id'], $_SESSION['user_role'], $_SESSION['user_name'], 'user_error', $errorDescription);
        }
    ...
    // AUDIT LOGGING - Log exception
    if (file_exists(__DIR__ . '/audit-logger.php')) {
        $errorDescription = 'Exception adding user: ' . $e->getMessage();
        logActivity($_SESSION['user_id'], $_SESSION['user_role'], $_SESSION['user_name'], 'user_error', $errorDescription);
    }
